...
const makeHref = (pathname: string, pName: string, locale?: string, href?: string) => {
  if (locale) {
...
const makePathname = (origin: string, query: ReadonlyURLSearchParams, locale: string) => {
  ...
  return (
    <>
      <a
        href={href}
      >
        <button style={{ fontSize: 'small' }} onClick={() => { if (languageDetector.cache) { languageDetector.cache(locale); } }}>
...
export default function LangSwitcher(): JSX.Element {
  ...
  const currentLocale = params.get('locale') as string || i18nextConfig.i18n.defaultLocale;
